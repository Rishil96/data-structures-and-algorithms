...
class Graph
{
    ...
    public:
        ...
        void floydWarshallAlgorithm(int n) {
            // 1. Create a distance array where every node is a source and a destination
            vector<vector<int>> dist(n, vector<int>(n, 10000));

            // 2. Mark all source distance to itself as 0
            for (int i=0; i<n; i++) dist[i][i] = 0;

            // 3. Add all distance according to the adjacency list
            for (auto node: adjList) {
            ...
            // 4. Use helper node to try and see if we can reach from source to destination faster via helper node
